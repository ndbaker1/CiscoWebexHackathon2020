...
// THIS IS WHAT WE ARE CODING
var urls = [];

framework.hears('validate', function (bot, trigger) {
  console.log(`validate called`);
  responded = true;
  bot.say(`Hello ${trigger.person.displayName}, the site ${trigger.args[1]} is ${helpers.validateURL(trigger.args[1])} percent valid.`)
    .then(() => sendHelp(bot))
    .catch((e) => console.error(`Problem in help hander: ${e.message}`));
});

framework.hears('citations', function (bot, trigger) {
  console.log(`citations called`);
  responded = true;
  bot.say(`Hello ${trigger.person.displayName}, citations:\n ${helpers.createCitations(urls)}`)
    .then(() => sendHelp(bot))
    .catch((e) => console.error(`Problem in help hander: ${e.message}`));
});
